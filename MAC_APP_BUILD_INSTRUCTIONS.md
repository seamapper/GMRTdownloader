...
# Install dependencies (PyQt6 only — do not install PyQt5 or PySide in this venv)
pip install PyQt6 requests numpy pyinstaller
pip install rasterio  # Optional but recommended
pip install netCDF4   # Optional but recommended
```

**Important:** Use a dedicated virtual environment for building. If `PyQt5`, `PySide2`, or `PySide6` are also installed, PyInstaller may try to run PyQt5 hooks and fail with errors like `No module named PyInstaller.hooks.hook-PyQt5.QtSql` even though this app uses PyQt6 only.

### Project structure (for building)

The app uses a modular layout. The build entry point is `GMRT_Downloader.py` (a thin launcher that calls `main.main()`). The real application lives in `main.py`, `config.py`, and the packages `ui/`, `workers/`, and `converters.py`. The **version** used for the app bundle (e.g. in Info.plist) should match `__version__` in `config.py`. The Mac app icon is `media/GMRT-logo2020.icns` (included in the repository; regenerate only if you change `GMRT-logo2020.png` — see [Creating an Icon File](#creating-an-icon-file)).
...
# Prevent PyInstaller from collecting other Qt bindings (app uses PyQt6 only)
qt_excludes = [
    'PyQt5', 'PySide2', 'PySide6',
    'PyQt5.QtCore', 'PyQt5.QtGui', 'PyQt5.QtWidgets', 'PyQt5.QtSql',
]
...
a = Analysis(
    ...
    hooksconfig={},
    runtime_hooks=[],
    excludes=qt_excludes + ['matplotlib', 'scipy', 'pandas'],
    noarchive=False,
    optimize=0,
)
...
```bash
# Optional but recommended: force PyQt6 as the only Qt binding for this build
export QT_API=PyQt6
...
Mac apps require `.icns` format icons. **You do not need to create this file for a normal build** — `media/GMRT-logo2020.icns` is already in the repository.

Regenerate icons only after you change `media/GMRT-logo2020.png`:
...
That script rebuilds both `GMRT-logo2020.ico` (Windows) and `GMRT-logo2020.icns` (macOS) from the PNG.
...
```text
ImportError: No module named PyInstaller.hooks.hook-PyQt5.QtSql
```

**Cause:** PyInstaller is trying to run PyQt5 hooks even though GMRT Downloader uses **PyQt6**. This usually means `PyQt5` (or another Qt package) is installed in the same environment, or PyInstaller itself is corrupted.

**Solution (try in order):**

1. **Use a clean build venv** with only PyQt6 (see [Required Python Packages](#required-python-packages)):
   ```bash
   python3 -m venv venv-build
   source venv-build/bin/activate
   pip install --upgrade pip pyinstaller PyQt6 requests numpy
   pip install rasterio netCDF4   # if you need those formats
   ```

2. **Remove other Qt packages** from the build environment:
   ```bash
   pip uninstall -y PyQt5 PyQt5-Qt5 PyQt5-sip PySide2 PySide6
   ```

3. **Reinstall PyInstaller** (fixes missing or broken hook files):
   ```bash
   pip install --force-reinstall pyinstaller
   ```

4. **Update your `.spec` file** so `Analysis(..., excludes=...)` includes the `qt_excludes` list from the template above.

5. **Build with Qt API pinned to PyQt6:**
   ```bash
   export QT_API=PyQt6
   pyinstaller GMRT_Downloader_Mac.spec
   ```
